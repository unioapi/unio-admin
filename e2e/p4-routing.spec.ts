    ...
    if (path === `/admin/v1/routes/${routeID}`) {
      await fulfillJSON(route, {
        data: {
          ...
          rpd_limit: null,
          sticky_enabled: null,
          description: "ProviderEndpoint 全局运行态",
          channels: [
            {
              channel_id: 7,
              channel_name: "主渠道",
        ...
        data: {
          ...
          channels: [
            runtimeChannel({
              channel_id: 7,
              channel_name: "主渠道",
              provider_endpoint_id: 11,
              provider_endpoint_name: "Primary Endpoint",
              eligible: false,
              excluded_reason: denied ? "store_unavailable" : "breaker_open",
              endpoint_breaker_state: denied ? null : "open",
              endpoint_open_remaining_ms: denied ? null : 12_400,
            ...
            runtimeChannel({
              channel_id: 9,
              channel_name: "版本落后渠道",
              provider_endpoint_id: 13,
              provider_endpoint_name: "Stale Endpoint",
              eligible: !denied,
            ...
            runtimeChannel({
              channel_id: 8,
              channel_name: "备用渠道",
              provider_endpoint_id: 12,
              provider_endpoint_name: "Backup Endpoint",
              eligible: !denied,
              excluded_reason: denied ? "store_unavailable" : undefined,
              endpoint_breaker_state: denied ? null : "closed",
              channel_breaker_state: denied ? null : "closed",
    ...
    if (path === "/admin/v1/system/runtime-diagnostics") {
      await fulfillJSON(route, {
        data: {
          ...
          operations: {
            endpoint_routing: { nonterminal_count: 0, oldest_age_seconds: null },
...
function runtimeChannel(overrides: Record<string, unknown>) {
  return {
    ...
    endpoint_base_url_revision: 3,
    endpoint_status_revision: 4,
    runtime_endpoint_base_url_revision: 3,
    runtime_endpoint_status_revision: 4,
    pending_endpoint_base_url_revision: null,
    pending_endpoint_status_revision: null,
    endpoint_base_url_revision_current: true,
    endpoint_status_revision_current: true,
    endpoint_state_generation: 2,
    endpoint_base_url_fence_generation: 2,
    endpoint_status_fence_generation: 2,
    ...
    endpoint_open_remaining_ms: null,
...
test("shows ProviderEndpoint breaker facts and stream-only TTFT", async ({
  page,
}) => {
